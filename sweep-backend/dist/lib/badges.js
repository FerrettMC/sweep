...
export async function collectBadgeStats(userId) {
    ...
    return {
        ...
        biggestDropPercent: dealTransactions.reduce((best, t) => Math.max(best, t.xpChange), 0),
        dealsOnFeed: feedDeals,
    };
}
export function badgesFor(stats) {
    const make = (id, label, description, icon, tier, current, target, unit) => ({
        id,
        label,
        description,
        icon,
        ...
        progressLabel: current >= target ? "Earned" : `${Math.min(current, target)} / ${target} ${unit}`,
    });
    return [
        make("first_find", "First Find", "Catch your first real price drop", "ribbon", "bronze", stats.dealsFound, 1, "deals"),
        make("deal_spotter", "Deal Spotter", "Catch 5 price drops", "eye", "bronze", stats.dealsFound, 5, "deals"),
        make("deal_hunter", "Deal Hunter", "Catch 25 price drops", "flame", "silver", stats.dealsFound, 25, "deals"),
        make("big_saver", "Big Saver", "Catch a drop 40% below average", "trending-down", "silver", stats.biggestDropPercent, 40, "%"),
        make("legendary_find", "Legendary Find", "Catch a drop 60% below average", "flash", "gold", stats.biggestDropPercent, 60, "%"),
        make("collector", "Collector", "Track 10 products at once", "albums", "bronze", stats.productsTracked, 10, "tracked"),
        make("trendsetter", "Trendsetter", "Have 3 of your finds hit the public feed", "megaphone", "silver", stats.dealsOnFeed, 3, "featured"),
        make("veteran", "Veteran", "Reach level 10", "trophy", "gold", stats.level, 10, "levels"),
    ];
}
