...
export async function getHealthReport(windowMinutes = WINDOW_MINUTES) {
    ...
    return Promise.all(RETAILERS.map(async (retailer) => {
        const rows = grouped.filter((g) => g.retailer === retailer);
        const countOf = (status) => rows.find((r) => r.status === status)?._count._all ?? 0;
        ...
        const lastDetail = failureRate > 0
...
export async function runHealthCheck() {
    ...
    for (const health of report) {
        ...
        const staleEnoughToRepeat = alert &&
            Date.now() - alert.lastAlertAt.getTime() >
                REALERT_AFTER_MINUTES * 60 * 1000;
        if (isOpenIncident && !staleEnoughToRepeat)
            continue;
