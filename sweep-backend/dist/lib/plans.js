...
import { TIERS, TIER_LIMITS } from "./tiers.js";
const PRICING = {
...
const DIALS = [
    ...
    {
        label: "Manual checks",
        value: (l) => l.manualChecksPerDay !== null
    ...
    {
        label: "Deal Radar",
        value: (l) => l.savedSearchIntervalMinutes === 0
 ...
 */
function unlocksFor(tier) {
    const previous = previousTier(tier);
    if (!previous)
        return [];
    const before = new Map(featuresFor(previous).map((feature) => [feature.label, feature.included]));
...
function featuresFor(tier) {
    ...
    return [
        ...
        },
        {
            group: "tracking",
            included: true,
            label: l.manualChecksPerDay !== null
        ...
        },
        {
            ...
            label: l.savedSearchIntervalMinutes > 0
        ...
        { group: "budget", included: true, label: "Unlimited expense logging" },
        {
            group: "budget",
            included: true,
            label: l.budgetHistoryMonths === null
...
function savingPercent(monthly, yearly) {
