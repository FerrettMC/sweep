...
import { TIER_LIMITS, effectiveTier } from "./tiers.js";
 ...
 */
export async function notifyPriceDrop({ productId, previousPrice, newPrice, }) {
    ...
    const messages = [];
    const notifiedTrackedIds = [];
    for (const tracked of trackers) {
        if (tracked.user.pushTokens.length === 0)
            continue;
        // Already told this user about this product recently.
        if (tracked.lastNotifiedAt && tracked.lastNotifiedAt > cooldownCutoff)
            continue;
...
async function send(messages) {
    ...
    for (const chunk of expo.chunkPushNotifications(messages)) {
        try {
            ...
            tickets.push(...chunk.map(() => ({ status: "error", message: "chunk send failed" })));
        }
    }
    return tickets;
}
 ...
 */
async function pruneDeadTokens(messages, tickets) {
    const dead = [];
