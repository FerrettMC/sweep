...
export function startScheduler() {
    ...
    console.log("[scheduler] started — price checks every 5m, radars every 10m, health sweep hourly");
}
        ...
        for (const drop of changed) {
            ...
            try {
                ...
                if (!verdict.believable) {
                    console.warn(`[scheduler] SUPPRESSED implausible drop ${drop.productId}: ${verdict.reason} (${verdict.detail})`);
                    continue;
                }
                ...
                if (awards.length > 0) {
                    console.log(`[scheduler] XP: ${awards.length} user(s) earned ${awards[0].award.xp} — ${awards[0].award.detail}`);
                }
                ...
                if (deal) {
                    console.log(`[scheduler] FEED: ${deal.percentBelowAverage}% below average`);
                }
    ...
    try {
        ...
        for (const saved of due) {
            try {
                ...
                const gap = effectiveIntervalMinutes(limits.savedSearchIntervalMinutes, foundSomethingNew ? 0 : saved.unchangedChecks + 1);
                await prisma.savedSearch.update({
                    where: { id: saved.id },
                    data: {
                ...
                if (foundSomethingNew && run.best) {
                    ...
                    if (sent > 0) {
                        ...
                        console.log(`[radar] "${saved.keyword}" → ${run.best.price} at ${run.best.retailerLabel}, notified ${sent}`);
                    }
                }
            }
            catch (err) {
