...
function config() {
    return {
        ...
        searchDataset: process.env.BRIGHTDATA_AMAZON_SEARCH_DATASET_ID ??
...
export async function scrapeAmazonProduct(url) {
    ...
    try {
        const rows = await runJob(`${API_BASE}/scrape?dataset_id=${productDataset}&format=json`, { input: [{ url }] }, apiKey);
...
export async function searchAmazonProducts(keyword, limit = 4) {
    const started = Date.now();
    ...
    try {
        const rows = await runJob(`${API_BASE}/scrape?dataset_id=${searchDataset}` +
            `&notify=false&include_errors=true&type=discover_new&discover_by=keyword`, { input: [{ keyword, zipcode: "" }], limit_per_input: limit }, apiKey);
        const products = (rows ?? [])
...
async function runJob(url, body, apiKey) {
    ...
    throw new BrightDataError(`Bright Data returned ${res.status}: ${errorBody.slice(0, 400)}`, res.status === 429 ? "blocked" : "failed");
}
// ---- shared: poll until the async job is ready, then fetch results ----
async function pollSnapshot(snapshotId, apiKey, maxAttempts = 40, delayMs = 5000) {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (!progressRes.ok) {
            throw new BrightDataError(`progress check failed: ${progressRes.status}`, "failed");
        }
        const progress = (await progressRes.json());
        if (progress.status === "ready") {
            const dataRes = await fetch(`${API_BASE}/snapshot/${snapshotId}?format=json`, {
                headers: { Authorization: `Bearer ${apiKey}` },
                signal: AbortSignal.timeout(60_000),
            });
    ...
    throw new BrightDataError(`job ${snapshotId} timed out after ${(maxAttempts * delayMs) / 1000}s`, "failed");
}
/**
...
function parseRows(body) {
    ...
    if (rows.length === 0) {
        throw new BrightDataError(`couldn't parse response as JSON or NDJSON: ${trimmed.slice(0, 200)}`, "failed");
    }
    return rows;
}
// ---- shared: normalize raw Bright Data fields into our shape ----
function parseAmazonProduct(raw, fallbackUrl) {
...
const NOT_CONFIGURED = "Amazon is not configured — set BRIGHTDATA_API_KEY and BRIGHTDATA_AMAZON_DATASET_ID (see docs/INTEGRATIONS.md)";
