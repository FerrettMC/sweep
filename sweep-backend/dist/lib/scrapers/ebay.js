...
export async function searchEbay(keyword, limit = 4) {
    ...
    try {
        ...
        const payload = await fetchJson(`${BROWSE_URL}/item_summary/search?${query}`, { headers: browseHeaders(token), retries: 1 });
...
export async function scrapeEbayProduct(url) {
    ...
    try {
        ...
        const item = await fetchJson(endpoint, {
...
export function ebayProductUrl(retailerId) {
...
async function getToken() {
    ...
    if (!res.ok) {
        throw new ScrapeHttpError(`eBay OAuth failed (${res.status}): ${(await res.text()).slice(0, 300)}`, "failed", res.status);
    }
...
// ---- parsing ---------------------------------------------------------------
function parseItemSummary(item) {
    const retailerId = item?.legacyItemId ?? item?.itemId;
    const title = item?.title;
    if (!retailerId || !title)
        return null;
    const price = toCents(item?.price?.value);
    const listPrice = toCents(item?.marketingPrice?.originalPrice?.value ?? item?.originalPrice?.value);
    return {
        retailer: "ebay",
...
const NOT_CONFIGURED = "eBay is not configured — set EBAY_CLIENT_ID and EBAY_CLIENT_SECRET (see docs/INTEGRATIONS.md)";
