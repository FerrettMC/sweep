...
import { ScrapeHttpError, extractNextData, fetchText, } from "./http.js";
import { fail, ok, toCents, } from "./types.js";
const BASE = "https://www.walmart.com";
export async function searchWalmart(keyword, limit = 4) {
    ...
    try {
        ...
        if (!Array.isArray(stacks)) {
            return fail("failed", "searchResult.itemStacks missing — page structure likely changed", elapsed(started));
        }
        // Walmart splits results across several stacks (organic, sponsored,
        // carousels); take the first one that actually holds items.
        const items = stacks.find((stack) => Array.isArray(stack?.items) && stack.items.length)
            ?.items ?? [];
...
export async function scrapeWalmartProduct(url) {
    ...
    try {
        ...
        if (!product) {
            return fail("failed", "initialData.data.product missing — page structure likely changed", elapsed(started));
        }
...
export function walmartProductUrl(retailerId) {
...
function parseSearchItem(item) {
    ...
    let price = toCents(priceLineValue(lines, "DISCOUNTED_PRICE", "PRICE")) ??
    ...
    let listPrice = toCents(priceLineValue(lines, "COMPARISON", "WAS_PRICE")) ??
    ...
    return {
        ...
        availability: normalizeAvailability(item?.availabilityStatusV2?.value ?? item?.availabilityStatus),
...
function parseProductPage(product, fallbackUrl) {
    ...
    const listPrice = toCents(priceInfo?.wasPrice?.price) ?? toCents(priceInfo?.listPrice?.price);
    return {
        retailer: "walmart",
        retailerId: String(retailerId),
        title: String(title),
...
function priceLineValue(lines, lineType, key) {
    if (!Array.isArray(lines))
        return null;
    const line = lines.find((l) => l?.lineType === lineType);
    const entry = line?.values?.find((v) => v?.key === key);
