...
export async function awardDealFound(params) {
    ...
    if (history.length < 3)
        return [];
    const average = Math.round(history.reduce((sum, point) => sum + point.price, 0) / history.length);
    if (average <= 0)
        return [];
    const percentBelow = Math.round(((average - newPrice) / average) * 100);
    if (percentBelow < MIN_DEAL_PERCENT)
        return [];
...
export async function awardFirstTrack(userId) {
 ...
 */
async function grant(userId, award, productId) {
