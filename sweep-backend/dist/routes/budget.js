...
import { MAX_AMOUNT_CENTS, MAX_CATEGORY_LENGTH, MAX_DESCRIPTION_LENGTH, availableCategories, DEFAULT_CATEGORIES, earliestReadableMonth, formatMonth, guessCategory, monthEnd, monthStart, parseMonth, } from "../lib/budget.js";
...
export async function budgetRoutes(app) {
    // ---- the month view ----
    app.get("/budget", { preHandler: requireAuth }, async (request, reply) => {
        ...
        if (earliest && month < earliest) {
            return reply.status(403).send({
                error: limits.budgetHistoryMonths === 1
        ...
        const categoryLimits = new Map(allLimits
    ...
    // ---- edit ----
    app.patch("/budget/:id", { preHandler: requireAuth }, async (request, reply) => {
        const userId = request.userId;
        const body = (request.body ?? {});
        const data = {};
    ...
    // ---- delete ----
    app.delete("/budget/:id", { preHandler: requireAuth }, async (request, reply) => {
    ...
    app.get("/budget/prefill/:productId", { preHandler: requireAuth }, async (request, reply) => {
        const product = await prisma.product.findUnique({
