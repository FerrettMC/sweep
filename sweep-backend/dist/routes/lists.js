...
export async function listRoutes(app) {
    ...
    // ---- rename / edit ----
    app.patch("/lists/:id", { preHandler: requireAuth }, async (request, reply) => {
        const userId = request.userId;
        const { name, description } = (request.body ?? {});
        const data = {};
        if (name !== undefined) {
    ...
    // ---- delete ----
    app.delete("/lists/:id", { preHandler: requireAuth }, async (request, reply) => {
    ...
    // ---- remove an item ----
    app.delete("/lists/:id/items/:itemId", { preHandler: requireAuth }, async (request, reply) => {
    ...
    // ---- sharing ----
    app.post("/lists/:id/share", { preHandler: requireAuth }, async (request, reply) => {
        const userId = request.userId;
    ...
    app.get("/shared/:token", { preHandler: optionalAuth }, async (request, reply) => {
    ...
    app.post("/shared/:token/items/:itemId/claim", async (request, reply) => {
