...
import { consumeRadarChange, consumeRadarRefresh, getRadarChangeState, getRadarRefreshState, } from "../lib/quota.js";
...
export async function radarRoutes(app) {
    // ---- list ----
    ...
    app.post("/radar", { preHandler: requireAuth }, async (request, reply) => {
        const userId = request.userId;
        const { keyword, targetPrice } = (request.body ?? {});
        ...
        if (count >= limits.maxSavedSearches) {
            return reply.status(403).send({
                error: `Your plan watches ${limits.maxSavedSearches} ${limits.maxSavedSearches === 1 ? "search" : "searches"} at a time.`,
        ...
        const saved = await prisma.savedSearch.create({
            data: {
                ...
                nextCheckAt: limits.savedSearchIntervalMinutes > 0
    ...
    // ---- edit ----
    app.patch("/radar/:id", { preHandler: requireAuth }, async (request, reply) => {
        const { keyword, targetPrice } = (request.body ?? {});
        const userId = request.userId;
    ...
    // ---- delete ----
    app.delete("/radar/:id", { preHandler: requireAuth }, async (request, reply) => {
            ...
            data: {
                ...
                ...(run.best
                    ? {
                        ...
                        lastBestPrice: saved.lastBestPrice === null
                            ? run.best.price
                ...
                ...(limits.savedSearchIntervalMinutes > 0
                    ? {
                        nextCheckAt: new Date(Date.now() + limits.savedSearchIntervalMinutes * 60_000),
                    }
                    : {}),
            },
        });
