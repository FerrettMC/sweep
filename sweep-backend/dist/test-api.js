...
async function signIn() {
    ...
    const data = signedIn.error
        ? await (async () => {
            ...
            if (created.error)
                throw new Error(`auth failed: ${created.error.message}`);
            return created.data;
        })()
        : signedIn.data;
    if (!data.session) {
        throw new Error("No session returned. Email confirmation is probably ON in Supabase — " +
...
async function main() {
    ...
    check("free tier gets 1 search/day", quota.body?.quota?.limit === 1, quota.body);
    console.log("\n— compiled search —");
    ...
    const successful = (search.body?.results ?? []).filter((r) => r.status === "success" && r.products.length > 0);
    check(`at least one retailer returned products (${successful.length} did)`, successful.length > 0, search.body?.results?.map((r) => `${r.retailer}:${r.status}`));
    ...
    const sample = successful.flatMap((r) => r.products);
    check("every product has a positive price", sample.length > 0 && sample.every((p) => typeof p.price === "number" && p.price > 0), sample.map((p) => [p.title?.slice(0, 30), p.price]));
    check("no product's price exceeds its list price", sample.every((p) => p.listPrice === null || p.price <= p.listPrice), sample
        .filter((p) => p.listPrice !== null && p.price > p.listPrice)
        .map((p) => [p.title?.slice(0, 30), p.price, p.listPrice]));
    console.log("\n— quota enforcement —");
    const second = await call("GET", `/search?q=laptop&retailers=walmart`);
    check("second search is refused once the daily cap is hit", second.status === 429 && second.body?.code === "SEARCH_LIMIT_REACHED", second.body);
    console.log("\n— tracking —");
    ...
    check("tracking the same product twice is idempotent", again.status === 201 && again.body?.tracked?.id === trackedId, again.body);
    const list = await call("GET", "/products");
    check("tracked list contains it", list.body?.tracked?.length >= 1, list.body);
    check("free tier cap is reported as 3", list.body?.limits?.maxTrackedProducts === 3, list.body?.limits);
    console.log("\n— detail + history —");
    const detail = await call("GET", `/products/${productId}`);
    check("detail loads", detail.status === 200, detail.body);
    check("history has a seeded point", (detail.body?.history?.length ?? 0) >= 1, detail.body?.history);
    check("stats are computed", detail.body?.stats?.average !== null, detail.body?.stats);
    check("free tier sees a 30-day window", detail.body?.historyWindow?.days === 30, detail.body?.historyWindow);
    console.log("\n— tier enforcement —");
    const threshold = await call("PATCH", `/products/track/${trackedId}`, {
        customThreshold: 5000,
    });
    check("custom thresholds are refused on the free tier", threshold.status === 403 && threshold.body?.code === "TIER_REQUIRED", threshold.body);
    ...
    check("refresh right after tracking is served fresh, not re-scraped", refresh.status === 200 && refresh.body?.status === "fresh", refresh.body?.status);
    check("a fresh result doesn't spend a manual check", refresh.body?.manualChecks?.used === 0, refresh.body?.manualChecks);
    check("free tier reports 5 manual checks a day", refresh.body?.manualChecks?.limit === 5, refresh.body?.manualChecks);
    console.log("\n— track limit —");
    ...
    if (overflow) {
        ...
        check("4th product is refused on the free tier", denied.status === 403 && denied.body?.code === "TRACK_LIMIT_REACHED", denied.body);
    }
    else {
    ...
    check("a malformed push token is rejected", malformedPush.status === 400 &&
        malformedPush.body?.code === "INVALID_PUSH_TOKEN", malformedPush.body);
    ...
    check("status reports the device", pushStatus.body?.registered === true && pushStatus.body?.devices >= 1, pushStatus.body);
    const reregister = await call("POST", "/notifications/register", {
        token: fakePushToken,
        platform: "android",
    });
    const afterRe = await call("GET", "/notifications/status");
    check("re-registering the same token doesn't duplicate it", reregister.status === 200 && afterRe.body?.devices === pushStatus.body?.devices, afterRe.body);
