...
function check(label, ok, detail) {
    if (ok) {
        ...
        console.log(`  ❌ ${label}`);
        if (detail !== undefined)
            console.log(`     ${JSON.stringify(detail).slice(0, 260)}`);
    }
}
async function signIn() {
    ...
    if (!data.session)
        throw new Error("no session — email confirmation is on?");
...
async function main() {
    ...
    check("the same link with tracking params is deduped, not double-tracked", dirty.status === 201 && dirty.body?.tracked?.id === firstId, { got: dirty.body?.tracked?.id, expected: firstId });
    ...
    check("an unsupported store is refused with a useful message", unsupported.status === 400 && unsupported.body?.code === "UNSUPPORTED_RETAILER", unsupported.body);
    const garbage = await call("POST", "/products/track", { url: "hello world" });
    check("non-link text is refused", garbage.status === 400 && garbage.body?.code === "INVALID_URL", garbage.body);
    ...
    if (productId) {
        ...
        check("it has a price and a history point", detail.body?.product?.price > 0 && detail.body?.history?.length >= 1, { price: detail.body?.product?.price, points: detail.body?.history?.length });
    }
    const list = await call("GET", "/products");
    check("exactly 2 distinct products tracked from 4 paste attempts", list.body?.tracked?.length === 2, list.body?.tracked?.map((t) => t.product.title?.slice(0, 30)));
