...
import type { TierLimits } from "./tiers.js";
...
const RETAILER_DEFAULT_CATEGORY: Record<string, string> = {
  bestbuy: "Electronics",
  newegg: "Electronics",
  asos: "Clothing",
};
 ...
 */
export function guessCategory(retailer: string, title: string): string {
 ...
 */
export function earliestReadableMonth(limits: TierLimits, now: Date): Date | null {
  if (limits.budgetHistoryMonths === null) return null;
  const start = monthStart(now);
  // `budgetHistoryMonths: 3` means this month plus the two before it.
  return new Date(
    Date.UTC(start.getUTCFullYear(), start.getUTCMonth() - (limits.budgetHistoryMonths - 1), 1),
  );
}
 ...
 */
export function availableCategories(used: string[]): string[] {
