...
export type MatchConfidence = "same" | "similar" | "unrelated";

export interface MatchResult {
  confidence: MatchConfidence;
  score: number;
  /** Plain-language reasons, shown to the user rather than kept internal. */
  caveats: string[];
}
...
function modelTokens(title: string): Set<string> {
  return new Set(
    tokens(title).filter((t) => /\d/.test(t) && t.length >= 2),
  );
}
...
function conditionsIn(title: string): string[] {
...
export function compareProducts(
  ...
  const conditionDiffers =
    candidateConditions.length > 0 && sourceConditions.length === 0;
  if (conditionDiffers) {
  ...
  if (score >= 0.72 && !blocking) confidence = "same";
  else if (score >= 0.4) confidence = "similar";
  else confidence = "unrelated";
 ...
 */
export function searchKeyFor(title: string): string {
