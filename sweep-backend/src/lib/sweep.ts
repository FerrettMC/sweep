...
import { compareProducts, searchKeyFor, type MatchConfidence } from "./matching.js";
import { prisma } from "./prisma.js";
import { routeQuery, searchAllRetailers } from "./scrapers/index.js";
import { RETAILER_LABELS, type Retailer, isRetailer } from "./scrapers/types.js";
...
export interface Alternative {
  ...
  price: number;
  confidence: Exclude<MatchConfidence, "unrelated">;
  /** Positive means cheaper than what you're looking at. */
  savings: number;
  caveats: string[];
}
...
export interface SweepResult {
  ...
  /** High-confidence same-product matches that cost less. */
  cheaperElsewhere: Alternative[];
  /** Weaker matches, explicitly not claimed to be identical. */
  similar: Alternative[];
  /** Stores we couldn't reach this time, so absence isn't read as "not sold there". */
  unreachable: string[];
  bestSaving: number;
  headline: string;
}

export async function sweepProduct(productId: string): Promise<SweepResult | null> {
  ...
  const claimedPercentOff =
    product.listPrice !== null && product.listPrice > price
  ...
  const cheaperElsewhere: Alternative[] = [];
  const similar: Alternative[] = [];
  ...
  if (others.length > 0) {
    ...
    for (const outcome of outcomes) {
      ...
      for (const candidate of outcome.products) {
        ...
        if (candidate.price < price * ABSURDLY_CHEAP) continue;
...
function buildHeadline(
  bestSaving: number,
  best: Alternative | undefined,
  sale: SweepResult["sale"],
): string {
