...
export async function productRoutes(app: FastifyInstance) {
  ...
  // ---- force a re-check ----
  // Pull-to-refresh on a product. Rate-limited per product because the user
  // pulling twice shouldn't cost two scrapes.
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (lastChecked && Date.now() - lastChecked.getTime() < 5 * 60 * 1000) {
        return reply.status(429).send({
          error: "Just checked — try again in a few minutes.",
          code: "REFRESH_TOO_SOON",
          lastCheckedAt: lastChecked,
        });
      }

      const outcome = await checkProduct(tracked.productId);
      const product = await prisma.product.findUnique({
